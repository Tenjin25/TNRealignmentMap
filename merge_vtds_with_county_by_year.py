...
def merge_vtds_with_counties(year):
    ...
    if os.path.exists(election_csv):
        ...
        if os.path.exists(election_csv):
            ...
            for _, row in df_elec.iterrows():
                ...
                if d > r:
                    win = "DEM"
                    label = f"D+{abs(round(pct, 1))}"
                elif r > d:
                    win = "REP"
                    label = f"R+{abs(round(pct, 1))}"
                else:
                    win = "TOSSUP"
                    label = "Tied"
                winner.append(win)
                margin_label.append(label)
            ...
            df_elec['margin_label'] = margin_label
    ...
    if __name__ == "__main__":
        import argparse
        parser = argparse.ArgumentParser(description="Merge VTDs with county and election data by year")
